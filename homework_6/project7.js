...
{	
	...
	for(let i = 0; i < springs.length; i++){
		let dir = positions[springs[i].p0].sub(positions[springs[i].p1]);
		let l = dir.len();
		dir.normalize();
		forces[springs[i].p0] = forces[springs[i].p0].add(dir.mul( -1 *(l - springs[i].rest) * stiffness))
		forces[springs[i].p1] = forces[springs[i].p1].add(dir.mul( (l - springs[i].rest) * stiffness))

		let rv = velocities[springs[i].p0].sub(velocities[springs[i].p1])
		let lv = rv.dot(dir)
		forces[springs[i].p0] = forces[springs[i].p0].add(dir.mul(-1 * lv * damping))
		forces[springs[i].p1] = forces[springs[i].p1].add(dir.mul(lv * damping))
	...
	for(let i = 0; i < positions.length; i++){
		for (const key of ['x', 'y', 'z']) {
			if(positions[i][key] <= -1){
				let h = positions[i][key] + 1;
				positions[i][key] = -1 + restitution * h;
				velocities[i][key] = -restitution * velocities[i][key]
			}
